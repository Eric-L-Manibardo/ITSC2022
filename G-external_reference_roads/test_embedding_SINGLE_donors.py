...
def train_model_mask(trainA,target,mask):

    trainA = selected_weeks(trainA, mask)

    target = selected_weeks(target, mask)
    
    df_train = build_dataset(trainA, target)
...
def test_model(inputA, test, model, scaler):
    '''
    ...
    df_test = build_dataset(inputA,  test)
...
for i in range(3):
    donor1 = pd.read_csv('B-colombia_LIMPIO_2018_2019/'+str(selected_loops.iloc[selec][i+1])+'.csv',index_col=0)
    ...
    model_mask_min, scaler_mask_min = train_model_mask(train1,train_target,mask_min)
    
    
    # con todos los datos
    # model_full, scaler_full = train_model_full(train1,train2,train3,train_target)
    
    y_test, y_pred_min = test_model(test1,test_target,model_mask_min,scaler_mask_min)
