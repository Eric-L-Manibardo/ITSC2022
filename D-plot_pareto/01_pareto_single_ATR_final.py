...
df = pd.read_csv('ND_results/aND_results_'+str(selec)+'.csv', index_col=False, header=None)
df = df.rename(columns={0: 'Mask_size', 1: 'RMSE'})

#normalizamos eje X
df['RMSE'] = df['RMSE'].div(best.loc[0][selec]/100)

df_full = pd.read_csv('full_results/afull_results_'+str(selec)+'.csv', index_col=False, header=None)
