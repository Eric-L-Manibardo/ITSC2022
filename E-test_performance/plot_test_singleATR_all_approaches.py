...
for row in ax:
    ...
    row.plot(y_pred_min)
    row.plot(y_pred_full)
    row.plot(y_pred_min_classic, linestyle='dashed')
    row.plot(y_pred_full_classic, linestyle='dashed')
    row.plot(y_test, color='k')
    row.set_ylabel('Flow')

    # row.set_xticklabels(eje_x[::frec], rotation=45, horizontalalignment='center')
        # row.set_xticklabels(range(len(eje_x))[::frec], eje_x[::frec], font='small', rotation=45, horizontalalignment='center')
    row.set_xticks(np.arange(len(eje_x))[::frec])
    row.set_xticklabels(eje_x[::frec])
