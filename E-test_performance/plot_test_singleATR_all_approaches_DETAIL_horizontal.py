...
# añadimos modelo naive t+1=t
y_naive=np.squeeze(test_target.values)
y_naive=y_naive[4:-1]
error_naive = sqrt(mean_squared_error(y_test,y_naive))
...
x=51
i=0
ax[i].plot(y_pred_min[96*x:96*(x+1)],c='tab:blue' )
ax[i].plot(y_pred_full[96*x:96*(x+1)],c='tab:orange')
ax[i].plot(y_pred_min_classic[96*x:96*(x+1)], linestyle='dashed', c='tab:green')
ax[i].plot(y_pred_full_classic[96*x:96*(x+1)], linestyle='dashed', c='tab:red')
...
ax[0].legend([
            ...
            'Approach 4',
            'Real data'
            ],loc='upper center', bbox_to_anchor=(0.95, 1.315),
